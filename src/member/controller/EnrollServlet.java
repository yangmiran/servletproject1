...
//		member.setHobby(joinHobby);
		...
		//처리 결과받기
		int result = new MemberService().insertMember(member); //insertMember create method로 MemberService.java에 소스 만들기
		
		//4. 받은 결과에 따라 성공/실패 뷰 선택해서 내보내기
		if(result > 0) {
			response.sendRedirect("/test1/views/member/loginPage.html");
		}else {
